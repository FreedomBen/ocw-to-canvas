...
class Video
  ...
  def initialize(hash)
    ...
    @captions_file = hash['captions_file']
  ...
  def self.from_orig_json(hash)
    # make sure we have a video.  if not raise an error
    unless hash['data_json']['resource_type'] == 'Video'
      raise 'This is not a video'
    end
    ...
    Video.new({
      ...
      'youtube_key' => hash['data_json']['youtube_key'],
      'captions_file' => hash['data_json']['captions_file'],
      'transcript_file' => hash['data_json']['transcript_file'],
      'thumbnail_file' => hash['data_json']['thumbnail_file'],
      'archive_url' => hash['data_json']['archive_url']
    })
  end
